(function () {
  ...
  const rodapeModuloTopo = querySelector('.rodape_modulotopo')

  const newsletterForms = ['#wf-form-optinEmail', '#wf-form-optinCel']
  ...
  for (const id of newsletterForms) {
    const form = querySelector(id, rodapeModuloTopo)
    ...
    const previousSiblingElement = form.previousElementSibling

    form.remove()

    previousSiblingElement.insertAdjacentHTML('afterend', form.outerHTML)
  }

  attachEvent(rodapeModuloTopo, 'submit', handleNewsletterFormSubmit, false)
